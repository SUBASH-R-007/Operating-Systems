...
int main() {
    ...
    while (completed < n) {
        // Find process with minimum BT among arrived and not yet done
        int idx = -1;
        int min_bt = 99999;
...
// For each process, input its Arrival Time (AT) and Burst Time (BT).

// Create a boolean array/list called Done of size n and set all its values to 0 (False) to track which processes have finished.
...
// Step 3: Find the Shortest Available Job

// Search through all processes to find the one that:

// Has already arrived (AT[i] <= current_time)

// Has not been executed yet (Done[i] == 0)

// Has the minimum Burst Time (BT) among all available processes.

// Tie-breaker condition: If two or more available processes have the same minimum Burst Time, select the one with the earliest Arrival Time.

// Let the index of this selected process be idx.

// Step 4: Handle CPU Idle Time

// If no process is found in Step 3 (meaning no uncompleted process has arrived by the current_time), the CPU is idle.
